window.addEventListener('DOMContentLoaded', () => {
  ...
  const update = () => { if (!sprite || isPaused) return; sprite.style.transform = `translate(${getV(iX, 0)}px, ${-getV(iY, 0)}px) scale(${getV(iSize, 100) / 100}) rotate(${getV(iDir, 90) - 90}deg)`; };
  [iX, iY, iSize, iDir].forEach(i => i && i.addEventListener('input', update));
  if (iName) iName.addEventListener('input', e => { if (badge) badge.textContent = e.target.value; const c = $('.asset-card.active span'); if (c) c.textContent = e.target.value; });
  ...
  const resetBtn = () => [run, pause].forEach(b => b && b.classList.remove('active')), togglePause = () => { if (!run || !run.classList.contains('active')) return; isPaused = !isPaused; if (pause) pause.classList.toggle('active', isPaused); if (canvas) canvas.classList.toggle('paused-state', isPaused); $('.asset-info-bar .info-input', true).forEach(i => i.disabled = isPaused); if (!isPaused) update(); };
  if (run) run.addEventListener('click', () => { if (isPaused) togglePause(); resetBtn(); run.classList.add('active'); if (stop) { stop.style.opacity = "1"; stop.style.pointerEvents = "auto"; } update(); });
  if (pause) pause.addEventListener('click', togglePause);
  if (stop) { stop.addEventListener('click', () => { if (isPaused) togglePause(); resetBtn(); [iX, iY].forEach(i => i.value = 0); iSize.value = 100; iDir.value = 90; update(); stop.style.opacity = "0.4"; stop.style.pointerEvents = "none"; }); stop.style.opacity = "0.4"; stop.style.pointerEvents = "none"; }

  const exec = (b) => { if (!b || isPaused) return; const t = b.textContent || "", inp = b.querySelector('.block-input'), v = inp ? parseFloat(inp.value) || 0 : 0; if (t.includes("歩")) { const r = (getV(iDir, 90) * Math.PI) / 180; if (iX) iX.value = Math.round(getV(iX, 0) + v * Math.cos(r)); if (iY) iY.value = Math.round(getV(iY, 0) + v * Math.sin(r)); } else if (t.includes("度")) { if (iDir) iDir.value = (getV(iDir, 90) + (b.querySelector('img')?.style.transform.includes('scaleX(-1)') === false ? v : -v)) % 360; } else if (t.includes("どこか")) { if (iX) iX.value = Math.floor(Math.random() * 300) - 150; if (iY) iY.value = Math.floor(Math.random() * 200) - 100; } else if (t.includes("x座標")) { if (iX) iX.value = v; } update(); };
  ...
  const touch = (b, p = false) => {
    b.addEventListener('touchstart', e => {
      if (e.target.classList.contains('block-input') || isPaused) return; e.stopPropagation(); const t = e.touches[0], r = b.getBoundingClientRect(); tx = t.clientX - r.left; ty = t.clientY - r.top;
      if (p) {
        // 🛠️ 【ワープの完全修正】パレットから新しく複製したブロック（クローン）を即座に「今回の移動ターゲット（activeBlk）」として上書き直結させ、ゾンビ現象を100%防止しました！
        const clone = b.cloneNode(true); clone.style.cssText = `position:fixed; z-index:1000; opacity:0.8; left:${t.clientX - tx}px; top:${t.clientY - ty}px;`;
        document.body.appendChild(clone); activeBlk = clone;
      } else {
        activeBlk = b; activeBlk.style.zIndex = '1000';
        if (b.parentElement.classList.contains('streech-block') || b.parentNode !== $('#streech-workspace')) {
          const ws = $('#streech-workspace'), wR = ws.getBoundingClientRect(); activeBlk.style.position = 'absolute'; activeBlk.style.left = `${r.left - wR.left}px`; activeBlk.style.top = `${r.top - wR.top}px`; ws.appendChild(activeBlk);
        }
      }
    }, { passive: true });

    b.addEventListener('touchmove', e => { if (activeBlk) { e.preventDefault(); const t = e.touches[0]; activeBlk.style.left = `${t.clientX - tx}px`; activeBlk.style.top = `${t.clientY - ty}px`; } }, { passive: false });

    b.addEventListener('touchend', e => {
      if (!activeBlk) return; const ws = $('#streech-workspace'), wR = ws.getBoundingClientRect(), bR = activeBlk.getBoundingClientRect(), pal = $('.block-palette'), palR = pal ? pal.getBoundingClientRect() : {left:0,right:0,top:0,bottom:0}, t = e.changedTouches[0];
      ...
      if (bR.right > wR.left && bR.left < wR.right && bR.bottom > wR.top && bR.top < wR.top + ws.offsetHeight) {
        ...
        ws.querySelectorAll('.streech-block').forEach(ex => { if (ex === activeBlk || ex.contains(activeBlk)) return; const exR = ex.getBoundingClientRect(), exX = exR.left - wR.left, exY = exR.top - wR.top; if (ex.classList.contains('wrap-block') && dX >= exX && dX <= exX + 165 && dY >= exY + 20 && dY <= exY + 55) { snap = ex.querySelector('.wrap-slot'); insideWrap = true; } else if (Math.abs(dX - exX) < 35 && Math.abs(dY - (exY + ex.offsetHeight)) < 35) snap = ex; });
        if (snap) { activeBlk.style.cssText = `position: relative; left: 0px; top: ${insideWrap ? 0 : 2}px; display: flex; flex-direction: column; z-index: 5; margin-${insideWrap ? 'left' : 'top'}: 2px;`; snap.appendChild(activeBlk); } else { if (activeBlk.parentElement !== ws) ws.appendChild(activeBlk); }
        if (p) touch(activeBlk, false);
      } else { activeBlk.remove(); }
      activeBlk = null;
    });
    // 🛠️ 【タップ実行のバグ修正】指を離したあとの余計な追従バグ（二重トリガー）を e.preventDefault() で完全に遮断！
    b.addEventListener('click', e => { if (!e.target.classList.contains('block-input') && !p && !isPaused) { e.preventDefault(); exec(b); } });
  };

  const refresh = () => $('.block-palette .streech-block', true).forEach(b => touch(b, true)), bindCard = (c) => c.addEventListener('click', () => { if (isPaused) return; $('.asset-card', true).forEach(card => card.classList.remove('active')); c.classList.add('active'); const n = c.querySelector('span').textContent; if (iName) iName.value = n; if (badge) badge.textContent = n; if (sprite) sprite.className = c.getAttribute('data-sprite-id') === "1" ? 'view-sprite1' : 'view-sprite-custom'; update(); });
  ...
  const addBtnEl = $('#add-sprite-btn'); if (addBtnEl && list) addBtnEl.addEventListener('click', () => { if (isPaused) return; const id = $('.asset-card', true).length + 1, name = `Sprite${id}`, c = document.createElement('div'); c.className = 'asset-card'; c.setAttribute('data-sprite-id', id); c.innerHTML = `<div class="asset-icon-placeholder"></div><span>${name}</span>`; list.appendChild(c); bindCard(c); c.click(); });
